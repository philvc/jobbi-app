...
const Button = ({ variant = "primary", text, onClick, type = "submit", ...rest }: ButtonProps) => {
	...
	return (
		<ChakraButton type={type} border={getBorder()} color={getTextColor()} _hover={{ bg: getBackgroundColor("hovered") }} _focus={{ border: getOutline() }} _active={{ bg: getBackgroundColor("focused") }} bg={getBackgroundColor()} onClick={onClick}>
			{text}
		</ChakraButton>
	);
};
